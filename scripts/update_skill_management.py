#!/usr/bin/env python3
"""Add or update the skill management entry in every user's skill.json.

The script walks the controller-mounted personalData directory, skips the
public template directory, and updates:

    <userId>/working/workspaces/default/skill.json

It inserts or replaces skills["技能管理"]. Other JSON values are preserved.
...
TARGET_SKILL: dict[str, Any] = {
    ...
    "metadata": {
        "name": "技能管理",
        "description": "技能发布、查询、安装管理技能。支持发布技能、查询技能列表和详情、下载安装技能等操作。",
        "version_text": "",
        "commit_text": "",
        "source": "customized",
        "protected": False,
        "requirements": {
            "require_bins": ["requests"],
            "require_envs": [],
        },
        "updated_at": "2026-08-19T08:21:04.939367Z",
    },
    "requirements": {
        "require_bins": ["requests"],
        "require_envs": [],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Add or update skills["技能管理"] in user skill.json files.'
    )
    parser.add_argument(
        "--personal-data-root",
    ...
    parser.add_argument(
        "--file-relative-path",
        default="working/workspaces/default/skill.json",
    ...
    parser.add_argument(
        "--backup",
        action="store_true",
        help="create skill.json.bak-YYYYmmddHHMMSS before writing",
    )
...
def patch_skill_text(text: str) -> tuple[str, bool]:
    original_json = json.loads(text)
    if not isinstance(original_json, dict):
        raise ValueError("root JSON value must be an object")

    root_props = parse_object_properties(text)
    skills_prop = require_unique_key(root_props, "skills", required=True)
    skills_text = text[skills_prop.value_start : skills_prop.value_end]
    original_skills = json.loads(skills_text)
    if not isinstance(original_skills, dict):
        raise ValueError("skills must be an object")

    skill_props = parse_object_properties(skills_text)
    target_prop = require_unique_key(skill_props, TARGET_SKILL_NAME, required=False)
    if target_prop is not None and original_skills.get(TARGET_SKILL_NAME) == TARGET_SKILL:
        return text, False

    newline = "\r\n" if "\r\n" in text else "\n"
    if target_prop is None:
        updated_skills_text = insert_object_property(
            skills_text,
            skill_props,
            TARGET_SKILL_NAME,
            TARGET_SKILL,
            skills_prop.key_indent,
            newline,
        )
    else:
        value_indent = target_prop.key_indent or skills_prop.key_indent + "  "
        replacement = format_json_value(TARGET_SKILL, value_indent, newline)
        updated_skills_text = (
            skills_text[: target_prop.value_start]
            + replacement
            + skills_text[target_prop.value_end :]
        )

    updated_text = (
        text[: skills_prop.value_start]
        + updated_skills_text
        + text[skills_prop.value_end :]
    )
    updated_json = json.loads(updated_text)

    expected_json = copy.deepcopy(original_json)
    expected_json["skills"][TARGET_SKILL_NAME] = TARGET_SKILL
    if updated_json != expected_json:
        raise RuntimeError("safety check failed: fields other than skills['技能管理'] changed")
    if updated_json["skills"].get(TARGET_SKILL_NAME) != TARGET_SKILL:
        raise RuntimeError("safety check failed: skills['技能管理'] was not updated")

    return updated_text, True


def insert_object_property(
    object_text: str,
    ...
    object_start = skip_ws(object_text, 0)
    if object_start >= len(object_text) or object_text[object_start] != "{":
        raise ValueError("expected a JSON object")
    object_end = skip_ws_reverse(object_text, len(object_text))
    if object_end <= object_start or object_text[object_end - 1] != "}":
        raise ValueError("unterminated JSON object")
    close_brace = object_end - 1

    child_indent = next((prop.key_indent for prop in props if prop.key_indent), "")
    if not child_indent:
        child_indent = parent_key_indent + "  "
    property_text = (
        json.dumps(key, ensure_ascii=False)
        + ": "
        + format_json_value(value, child_indent, newline)
    )

    if not props:
        inner_whitespace = object_text[object_start + 1 : close_brace]
        if newline in inner_whitespace:
            insertion = newline + child_indent + property_text
            return object_text[: object_start + 1] + insertion + object_text[object_start + 1 :]
        replacement = newline + child_indent + property_text + newline + parent_key_indent
        return object_text[: object_start + 1] + replacement + object_text[close_brace:]

    last = props[-1]
    whitespace_before_close = object_text[last.value_end : close_brace]
    insertion = "," + newline + child_indent + property_text
    if newline in whitespace_before_close:
        return object_text[: last.value_end] + insertion + object_text[last.value_end :]

    insertion += newline + parent_key_indent
    return object_text[: last.value_end] + insertion + object_text[close_brace:]
...
def skip_ws_reverse(text: str, index: int) -> int:
...
def main() -> int:
    ...
    for user_id in users:
        ...
        try:
            ...
            updated_text, should_update = patch_skill_text(text)
